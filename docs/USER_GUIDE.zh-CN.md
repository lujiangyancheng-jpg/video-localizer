...
下载 `YouTube-Chinese-Localizer-0.7.0.15-Standard-Offline-Setup.exe` 后直接双击安装即可。Standard 已是单文件安装包，
不再需要额外的 `.bin`。它包含 Python、精简 FFmpeg、字幕字体和两套快速翻译模型；
安装过程会出现“选择本地模型”页，可按需勾选 Whisper Small（多数电脑推荐）、Whisper Medium（更高识别质量）
、本地 AI 段落翻译所需的 Qwen3:4b 与 Ollama，以及可选 AI 超分辨率组件。全新安装默认勾选 Whisper Small；安装器只会下载所勾选的组件并逐个校验 SHA-256，已有组件升级时不会自动重复下载。若主动取消全部 Whisper 选项，安装器会确认该安装只能下载无字幕视频。四段版本号的最后一段是程序迭代号，例如 `0.7.0.15` 继续使用 `0.7.0` 组件包。
...
启动软件后不会自动弹出教程。需要帮助时点击主界面的“帮助中心”：其中的“快速开始”说明完整流程，“环境与模型”显示 Whisper、本地 AI、性能调度和输出磁盘状态，并可打开“管理本地模型与画质组件”查看每个组件的安装状态、残缺提示和磁盘占用；“常见问题”提供限流、显存、磁盘与字幕压制失败的恢复方法。缺少模型时，该页面会打开前三段兼容的模型 Release，而不是错误地跳到不含模型的程序小迭代页面。
...
5. 选择实际版权依据并填写需要的权利人、许可链接或书面授权编号；如果需要字幕，再选择翻译方式。
6. 点击“开始本地化”。

### 3.8 AI 超分辨率

在“调整处理设置”中可选择“通用实拍”或“动画线稿”。默认关闭，适合原片已经足够清晰、只想保留原始码流的情况。开启后，本地模型会逐帧降噪与放大；自动输出默认 2 倍并封顶 4K，选择更高目标时最多使用一次 4 倍处理。

超分辨率不能找回源视频中真实不存在的文字、纹理或人脸细节，也可能产生人工纹理。它会显著增加耗时、显卡负载、临时空间和一次视频编码。项目处理完成或中断后，可选中任务并点击“超分对比”，生成约 10 秒的左侧原画／右侧 AI 增强视频；窗口会显示本次耗时和估算的整片时间。命令行也可运行 `python main.py enhancement-preview "项目目录" --start 60 --duration 10`。

程序使用小批次流式处理并自动尝试多个 Vulkan 设备，在某块显卡运行库不兼容时会切换到下一可用设备。每个新片段开始前都会核对剩余磁盘；检查点只有在最终成片通过校验并写入完整性记录后才会清理，因此空间不足或意外中断后可以继续，而不会重做全部视频。

无字幕下载也可以开启超分，此时输出位于项目 `rendered\enhanced_source.mp4`。关闭超分且选择“保持原视频”时直接保留最高质量原片、不重新编码；如果明确选择更低分辨率或更低帧率，则只转换一次并把对应结果写入 `rendered\enhanced_source.mp4`。

### 3.9 版权记录

开始前必须选择原创、书面授权、Creative Commons、公共领域或其他依据。项目的 `publishing` 文件夹会生成 `RIGHTS_RECORD.json`、`RIGHTS_RECORD.md` 和 `ATTRIBUTION.txt`。前两项用于保存本次声明和缺失字段，最后一项可复制到发布简介。

这些文件只记录你填写的内容，不会证明你确实拥有权利。特别要核对翻译／改编权、重新发布权、商业用途、署名、许可链接和 ShareAlike 要求。完整规则见项目根目录的 `TERMS_OF_USE.md` 与 `COPYRIGHT_POLICY.md`。
...
- “仅下载原视频（无字幕）”始终先取得最高画质视频和最高质量音频并自动合并，完成后直接停下，不运行 Whisper、翻译或字幕压制；保持原规格时文件位于项目的 `source` 文件夹，选择较低分辨率／帧率时另在 `rendered` 中生成限制后的版本。
...
桌面端在队列失败、单独暂停项目或点击“停止任务”后，会启用“重试未完成任务”。也可在任务中心选中一项后点击“继续所选”。恢复只重新提交对应的失败、暂停或未启动项目，
...
并会隐藏路径、链接和凭证。可识别的任务失败也会自动在对应项目目录生成同类诊断包。命令行也可以使用：
...
报告会列出阅读速度过快、闪现时间少于 0.7 秒、行过长、连续重复、数字变化和词汇表术语遗漏的字幕编号；阅读速度规则覆盖中文、英文、日语、韩语和阿拉伯语。它只提示人工
复核，不会擅自改写翻译或时间轴。
